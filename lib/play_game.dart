...
class _LobbyPageState extends State<LobbyPage> {
  ...
  var newGame = {
    "players": [FirebaseAuth.instance.currentUser!.uid], // aktualnie zalogowany użytkownik jako biały
...
Future<bool> joinGame(String gameId) async {
  ...
  if (gameSnap.exists && (gameSnap.data()! as Map<String, dynamic>)['players'].length == 1 && (gameSnap.data()! as Map<String, dynamic>)['status'] == 'waiting') {
  ...
  void initState() {
    super.initState();
    _gamesStream = FirebaseFirestore.instance.collection('games').where('status', isEqualTo: 'waiting').snapshots();
  }
        ...
        children: [
          ElevatedButton(
            onPressed: () async {
              ...
              Navigator.push(context, MaterialPageRoute(builder: (_) => GameBoard(gameId: gameId)));
            },
              ...
              builder: (context, snapshot) {
                ...
                return ListView(
                  children: snapshot.data!.docs.map((DocumentSnapshot document) {
                    Map<String, dynamic> game = document.data()! as Map<String, dynamic>;
                    return ListTile(
                      ...
                      onTap: () async {
                        ...
                        if (joined) {
                          Navigator.push(context, MaterialPageRoute(builder: (_) => GameBoard(gameId: document.id)));
                        } else {
                          ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text("Failed to join the game")));
                        }
                      },
                    );
                  }).toList(),
